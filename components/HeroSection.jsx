...
const locations = ["Delhi", "Patiala", "Chandigarh", "Shimla", "Manali"];
...
export default function HeroSection() {
  ...
  function onSubmit(data) {
    ...
    const route = destinations.find(r => 
  ...
  return (
    <section className="max-w-6xl mx-auto px-5 py-8 grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-gradient-to-br from-slate-900 to-slate-800 p-6 rounded-2xl shadow-md w-full mx-auto">
        <h1 className="text-3xl font-extrabold text-cyan-400 mb-6 text-center lg:text-left">Find Your Cab</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div>
            <label className="block text-sm text-gray-300 mb-2 font-medium">From</label>
            <select 
              {...register("from")} 
            ...
            {errors.from && <p className="text-red-500 mt-1 text-xs">{errors.from.message}</p>}
          </div>
          
          <div>
            <label className="block text-sm text-gray-300 mb-2 font-medium">To</label>
            <select 
              {...register("to")} 
              className="w-full rounded-lg p-3 text-sm bg-gray-800 text-white border border-gray-700 focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/20 transition-all"
            >
            ...
            {errors.to && <p className="text-red-500 mt-1 text-xs">{errors.to.message}</p>}
          </div>
        </div>
